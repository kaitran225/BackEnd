...
public class SurveyService {
    ...
    private SurveyResultsResponse mapToListResultsResponse(Surveys survey, List<StatusStudentResponse> std, String periodicID) {
    ...
    public List<PeriodicResponse> getPeriodicResults(String studentID) {
        ...
        List<SurveyResultsResponse> surveyResults = sr.stream().map(surveyResult -> {
            ...
            LocalDateTime completionDate = surveyResult.getCompletionDate();
            List<Periodic> periodics = survey.getPeriodic();
            String periodicID = "";
            for (Periodic periodic : periodics) {
                if (completionDate.isAfter(periodic.getStartDate()) && completionDate.isBefore(periodic.getEndDate())) {
                    periodicID = periodic.getPeriodicID();
                    break;
                }
            }
            return mapToListResultsResponse(
                    survey,
                    List.of(getStatusStudent(survey, surveyResult)), periodicID);
        }).toList();
    ...
    public SurveyQuestionResponse getReport(String surveyID, String studentId, String periodID) {
        ...
        for (SurveyResult result : surveyResultList) {
            ...
            if (result.getCompletionDate().isAfter(periodic.getStartDate()) && result.getCompletionDate().isBefore(periodic.getEndDate())) {
                surveyResult = result;
                break;
            }
        }
        ...
        SurveyResult finalSurveyResult = surveyResult;
        List<QuestionResponse> questionList = surveyQuestions
                .stream()
                .map(questions -> {
                            List<QuestionOption> questionOption = getQuestionResponse(questions);
                            questionOption.forEach(option -> {
                                if (option.getAnswerID().equals(
                                        finalSurveyResult.getChoices().get(
                                                surveyQuestions.indexOf(questions)).getOptionID())
                                ) {
                                    option.setChecked(true);
                                }
